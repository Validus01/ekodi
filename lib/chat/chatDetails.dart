...
class _ChatDetailsState extends State<ChatDetails> {
  ...
  sendMessage(Account sender, Account receiver) async {
    List<Message> messages = await MessageProvider().getDMMessages(sender, receiver);

    Message message = Message(
      messageID: DateTime.now().millisecondsSinceEpoch.toString(),
      senderID: sender.userID,
      chatID: messages.isEmpty ? sender.userID! +"_"+ receiver.userID! : messages.last.chatID,
      imageUrl: "",
  ...
  AppBar _buildAccountAppBar(Account account) {
    return AppBar(
      ...
      title: ListTile(
        ...
        subtitle: Text(account.phone!, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(color: Colors.white),),
      ),
      actions: [
        IconButton(
          onPressed: () {
                                ...
                                child: Container(
                                  decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.only(
                                        topLeft: isMine ? const Radius.circular(10.0) : const Radius.circular(0.0),
                                        topRight: const Radius.circular(10.0),
                                        bottomLeft: const Radius.circular(10.0),
                                        bottomRight: isMine ? const Radius.circular(0.0) : const Radius.circular(10.0),
                                      ),
                                      boxShadow: const [
                                        BoxShadow(
                                  ...
                                  child: Padding(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Padding(
                                          padding: const EdgeInsets.all(5.0),
                                          child: Text(message.messageDescription!, maxLines: 50, overflow: TextOverflow.ellipsis,),
                                        ),
                                        const SizedBox(height: 2.0,),
                                        Text(DateFormat("HH:mm, dd MMM").format(DateTime.fromMillisecondsSinceEpoch(message.timestamp!)), style: TextStyle(color: Colors.grey, fontSize: 10.0),)
                                      ],
                                    ),
                                  ),
                                ),
